...
class OpenEhrRestClient {
         ...
         else if (response_json._type == 'query_result_projections')
         {
            ...
            response_json.result.each { item ->
               ...
               item.projections.each { projection ->
                  ...
                  parsed_item_list_row << new QueryResultProjection(
                     ...
                     archetypeId: projection.archetype_id,
                     path:        projection.path,
                     dataPath:    projection.data_path, // helps grouping individual nodes into RM subtrees
                     value:       dv
                  )
               }
               ...
               parsed_item_list << new QueryResultItemProjections(
                  ...
                  ownerUid:      item.meta.owner_uid,
                  ehrUid:        item.meta.ehr_uid,
                  subjectId:     item.meta.subject_id,
                  timeCommitted: parseDate(item.meta.time_committed)
               )
            }
            ...
            response_json.headers.each { header ->
               parsedHeaders << new QueryResultHeader(
                  archetypeId: header.archetype_id,
                  path:        header.path,
                  name:        header.name,
   ...
   */
   ...
   DvText parse_DV_TEXT(Map dv)
   {
      new DvText(value: dv.value)
   }
   ...
   DvDateTime parse_DV_DATE_TIME(Map dv)
   {
      new DvDateTime(value: dv.value)
   }

   DvDate parse_DV_DATE(Map dv)
   {
      new DvDate(value: dv.value)
   }

   DvTime parse_DV_TIME(Map dv)
   {
      new DvTime(value: dv.value)
   }

   DvBoolean parse_DV_BOOLEAN(Map dv)
   {
      new DvBoolean(value: dv.value)
   }

   DvIdentifier parse_DV_IDENTIFIER(Map dv)
   {
      new DvIdentifier(
         issuer: dv.issuer,
         assigner: dv.assigner,
         id: dv.id,
         type: dv.type
      )
   }

   DvUri parse_DV_URI(Map dv)
   {
      new DvUri(value: dv.value)
   }

   DvEhrUri parse_DV_EHR_URI(Map dv)
   {
      new DvEhrUri(value: dv.value)
   }

   DvMultimedia parse_DV_MULTIMEDIA(Map dv)
   {
      new DvMultimedia(
         alternateText: dv.alternateText,
         uri: dv.uri,
         mediaType: parse_CODE_PHRASE(dv.mediaType),
         compressionAlgorithm: parse_CODE_PHRASE(dv.compressionAlgorithm),
         integrityCheckAlgorithm: parse_CODE_PHRASE(dv.integrityCheckAlgorithm),
         integrityCheck: dv.integrityCheck ? dv.integrityCheck.decodeBase64() : null,
         size: dv.size,
         width: dv.width,
         height: dv.height
      )
   ...
   DvParsable parse_DV_PARSABLE(Map dv)
   {
      new DvParsable(
         value: dv.value,
         formalism: dv.formalism
      )
   }
